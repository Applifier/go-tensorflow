...
func (*PlatformConfig) Descriptor() ([]byte, []int) { return fileDescriptorPlatformConfig, []int{0} }
...
type PlatformConfigMap struct {
	...
	PlatformConfigs map[string]*PlatformConfig `protobuf:"bytes,1,rep,name=platform_configs,json=platformConfigs" json:"platform_configs,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value"`
}
...
func (*PlatformConfigMap) Descriptor() ([]byte, []int) { return fileDescriptorPlatformConfig, []int{1} }
...
func (m *PlatformConfig) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.SourceAdapterConfig != nil {
		dAtA[i] = 0xa
		i++
		i = encodeVarintPlatformConfig(dAtA, i, uint64(m.SourceAdapterConfig.Size()))
		n1, err := m.SourceAdapterConfig.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n1
...
func (m *PlatformConfigMap) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.PlatformConfigs) > 0 {
		for k, _ := range m.PlatformConfigs {
			dAtA[i] = 0xa
			i++
			...
			if v != nil {
				msgSize = v.Size()
				msgSize += 1 + sovPlatformConfig(uint64(msgSize))
			}
			mapSize := 1 + len(k) + sovPlatformConfig(uint64(len(k))) + msgSize
			i = encodeVarintPlatformConfig(dAtA, i, uint64(mapSize))
			dAtA[i] = 0xa
			i++
			...
			if v != nil {
				dAtA[i] = 0x12
				i++
				i = encodeVarintPlatformConfig(dAtA, i, uint64(v.Size()))
...
func (m *PlatformConfig) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if m.SourceAdapterConfig == nil {
				m.SourceAdapterConfig = &google_protobuf.Any{}
			}
	...
	for iNdEx < l {
		...
		case 3:
			for {
				...
				next, err := skipPlatformConfig(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
...
func init() {
	proto.RegisterFile("tensorflow_serving/platform_config.proto", fileDescriptorPlatformConfig)
}

var fileDescriptorPlatformConfig = []byte{
	...
	0xc1, 0xe8, 0x24, 0x7c, 0xe2, 0x91, 0x1c, 0xe3, 0x85, 0x47, 0x72, 0x8c, 0x0f, 0x1e, 0xc9, 0x31,
	0x4e, 0x78, 0x2c, 0xc7, 0xf0, 0x83, 0x91, 0x31, 0x89, 0x0d, 0x1c, 0x38, 0xc6, 0x80, 0x00, 0x00,
	0x00, 0xff, 0xff, 0x90, 0xa6, 0xd1, 0x53, 0xb7, 0x01, 0x00, 0x00,
}
