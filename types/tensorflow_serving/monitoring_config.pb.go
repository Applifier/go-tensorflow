...
func (m *MonitoringConfig) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.PrometheusConfig != nil {
		{
			size, err := m.PrometheusConfig.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
			i = encodeVarintMonitoringConfig(dAtA, i, uint64(size))
		}
		i--
		dAtA[i] = 0xa
	}
...
func skipMonitoringConfig(dAtA []byte) (n int, err error) {
	...
	for iNdEx < l {
		...
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupMonitoringConfig
			}
			depth--
		case 5:
			iNdEx += 4
